...
python3 - "$RELEASE_NAME" "$SHA" "$GITHUB_REPOSITORY" <<'PY'
from pathlib import Path
import re
import sys

release_name, sha, repo = sys.argv[1:4]
...
stanza = f'''
  <li><a href="https://github.com/{repo}/raw/{sha}/circuitikzgit.sty">
  Version {version} ({release_date})</a>; also the corresponding
  <a href="https://github.com/{repo}/blob/{sha}/ctikzstylesgit.zip?raw=true">
  styles </a> and
  <a href="https://github.com/{repo}/blob/{sha}/circuitikzmanualgit.pdf?raw=true">
  manual</a>.
</li>'''
...
if f"Version {version} ({release_date})" in managed_block:
    ...
    print(f"Inserted Version {version} ({release_date}) for {sha}")
PY
...
git commit -m "Deploy release ${RELEASE_TAG}: ${SHA}"
